...
// Tu código acá:
document.querySelector('#createdBy').innerHTML = 'Aplicacion creada por Jeniffer Hurtado';
...
//toDoItems.push(new ToDo('prueba'));
//toDoItems.push(new ToDo('otra prueba'));
//toDoItems[1].completeToDo();
...
function buildToDos(toDos) {
  // Tu código acá:
  return toDos.map(function(elem, index) {
...
function displayToDos(toDos) {
  ...
  buildToDos(toDos).forEach(function(elem, index) {
...
chk.addEventListener('change', () => {
	document.body.classList.toggle('body_dark--mode');
